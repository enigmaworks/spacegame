export class Animation {
    constructor (initial, goal, time, oncomplete = ()=>{},repeat = false,reverseOnEnd = false) {
    ...
    step(delta){
        ...
        if (this.animationValue >= this.endingValue || this.animationValue <= this.startingValue) {
            if(this.doesRepeat){
                if(this.doesReverse){
                    this.direction *= -1;
...
export let AnimationHandler = {
    ...
    create(name,start,end,length, options = {}){
        ...
        let animation = new Animation(start,end,length,onEnd,loop,smoothLoop);
        let animationObject = {
            name: name,
            animation: animation,
        }
